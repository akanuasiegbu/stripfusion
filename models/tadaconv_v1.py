...
class RouteFuncMLP(nn.Module):
    ...
    def __init__(self, c_in, c_out, ratio, kernels, bn_eps=1e-5, bn_mmt=0.1):
        """
        Args:
        ...
        self.bn = nn.BatchNorm3d(int(c_out//ratio), eps=bn_eps, momentum=bn_mmt)
    ...
    def forward(self, x):
        # g = self.globalpool(x)
        # x = self.avgpool(x)
        g = x.mean(dim=(2,3,4)).unsqueeze(2).unsqueeze(3).unsqueeze(4)
        x = x.mean(dim=(3,4)).unsqueeze(3).unsqueeze(4)
        x = self.a(x + self.g(g))
        x = self.bn(x)
        x = self.relu(x)
        x = self.b(x) + 1
...
class TAdaConv2d(nn.Module):
    ...
    def forward(self, x, alpha):
        ...
        if self.cal_dim == "cin":
            ...
            weight = (alpha.permute(0,2,1,3,4).unsqueeze(5) * self.weight).reshape(-1, c_in//self.groups, kh, kw)
