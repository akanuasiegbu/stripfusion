...
#             x[i] = self.m[i](x[i].permute(2,0,1,3,4)[0])  # conv
...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, numframes=1, nc=None, anchors=None, use_tadaconv=False):  # model, input channels, number of classes
        ...
        if use_tadaconv:
            ...
            # NJOTWANI
            self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch], nf=numframes)  
        ...
        for m in ms:
            ...
            if  isinstance(m, ThermalRgbDetectv2): 
                s = 256  # 2x min stride
                # print("1, ch, s, s", 1, ch, s, s)
                # m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s), torch.zeros(1, ch, s, s))])  # forward
                m.stride = torch.Tensor([8.0, 16.0, 32.0])
    ...
    def forward(self, x, augment=False, profile=False, use_tadaconv=False, numframes=3, pp_fusion_nms=False):
        # thermal RGB split
        x2 = x[:,3:]
        x = x[:,:3]
        if augment:
            ...
            for si, fi in zip(s, f):
                ...
                yi = self.forward_once(xi, use_tadaconv=use_tadaconv, numframes=numframes, pp_fusion_nms=pp_fusion_nms)[0]  # forward
            ...
            return self.forward_once(x, x2, profile, use_tadaconv=use_tadaconv, numframes=numframes, pp_fusion_nms=pp_fusion_nms)  # single-scale inference, train


    def forward_once(self, x, x2, profile=False, use_tadaconv=False, numframes=3, pp_fusion_nms = False, change_shape = (10, 17, 26, 60)):
        """

        :param x:          RGB Inputs
        :param x2:         IR  Inputs
        :change_shape: corresponds to fusion and lastframe detector head
        :param profile:
        :return:
        """
        y, dt = [], []  # outputs
        ...
        for m in self.model:
            ...
            if m.i == 0:
                fusion_args= [self.model[reshape] for reshape in change_shape[:-1]] # to control arguments into the fusion modules 
                detection_head_args = [self.model[change_shape[-1]] ] # to control arguments into detection layer
            
            if m.f == -4:
                x = m(x2)
            else:
                if m in fusion_args:
                    x = m(x, use_tadaconv=use_tadaconv, numframes=numframes)
                elif m in detection_head_args:
                    x = m(x, use_tadaconv=use_tadaconv, numframes=numframes, pp_fusion_nms=pp_fusion_nms)
                else:
                    x = m(x)  # run
...
# Added modules and layers to match with parse_model_tadaconv() NJOTWANI 
def parse_model(d, ch, nf=3):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(layer_list):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
            ...
            c2 = ch[f[0]]
            args = [c2]
        elif m is  MLPSpatialConcatMixer:
            args =[*args]
        elif m is Detect:
            ...
            args = [*args, nf]
        elif m in [LastFrameThermalRgbDetectv2]: 
            args = [*args]
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
        else:
...
def parse_model_tadaconv(d, ch, nf=1, use_tadaconv=True):  # model_dict, input_channels(3) # Added use_tadaconv NJOTWANI
    logger.info('\n%3s%18s%3s%10s  %-40s%-30s' % ('', 'from', 'n', 'params', 'module', 'arguments'))
    anchors, nc, gd, gw = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple']
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors
    no = na * (nc + 5)  # number of outputs = anchors * (classes + 5)
    ...
    for i, (f, n, m, args) in enumerate(layer_list):  # from, number, module, args
        ...
        n = max(round(n * gd), 1) if n > 1 else n  # depth gain
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP,SPPv2, SPPv3, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 C3, C3TR]:
            ...
            if m is Focus:
                ...
                if use_tadaconv and m in [SPP, Conv]:
                    args = [c1, c2, *args[1:], nf]
                else:
                    args = [c1, c2, *args[1:]]
        ...
        if m is not C3:
            ...
            m_ = nn.Sequential(*[m(*args, num_frames=nf) for _ in range(n)]) if n > 1 else m(*args, num_frames=nf)  # module
        t = str(m)[8:-2].replace('__main__.', '')  # module type
        np = sum([x.numel() for x in m_.parameters()])  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        logger.info('%3s%18s%3s%10.0f  %-40s%-30s' % (i, f, n, np, t, args))  # print
