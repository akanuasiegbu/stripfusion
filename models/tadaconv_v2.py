...
from .misc_tadaconv_v2 import LayerNorm, QuickGELU
from .init_helper_tadaconv_v2 import trunc_normal_
from .tadaconv_v1 import RouteFuncMLP
...
class RouteFuncwTransformer(nn.Module):
    ...
    def __init__(self, c_in, c_out, ratio, kernels, with_bias_cal=False, bn_eps=1e-5, bn_mmt=0.1, zero_init_cal=True, head_dim=16):
        """
        Args:
        ...
        self.a = nn.Conv3d(
            in_channels=c_in,
            out_channels=int(c_out//ratio),
            kernel_size=[kernels[0],1,1],
            padding=[kernels[0]//2,0,0],
        )

        self.norm = LayerNorm(int(c_out//ratio), eps=1e-6, data_format="channels_first")
        self.norm_transformer = LayerNorm(int(c_out//ratio), eps=1e-6, data_format="channels_first")
        self.gelu = QuickGELU()

        self.scale = int(c_out//ratio) ** -0.5
        self.qkv_proj = nn.Conv3d(
            in_channels = int(c_out//ratio),
            out_channels = int(c_out//ratio)*3,
            kernel_size=1,
            padding=0,
        )
    ...
    def forward(self, x):
        # x = self.avgpool(x)
        x = x.mean(dim=(3,4)).unsqueeze(3).unsqueeze(4) #prevents infs that lead to nans
        x = self.a(x)
        x = self.norm(x)
        x = self.gelu(x)
...
class TAdaConv2dV2(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, num_frames=None, padding=None, dilation=1, groups=1, bias=False,
                 cal_dim="cin", rf_r=4, rf_k=[3,3], head_dim=16, 
        ...
        if internal_temp_aggr:
            ...
            if num_frames is not None and num_frames == 1:
                self.avgpool = nn.AvgPool3d(kernel_size=(1,1,1),stride=(1,1,1),padding=(0,0,0))
            # self.avgpool = nn.AvgPool3d(kernel_size=(1,1,1),stride=(1,1,1),padding=(0,0,0))
    ...
    @staticmethod
    def cumprod(feat, dim=2):
        padding = [0, 0] * (feat.dim() - dim - 1) + [1, 1]
        input_padded = F.pad(feat, padding, 'constant', 1)
        windows = input_padded.unfold(dim, 3, 1)
        res = windows.permute(0,1,2,5,3,4).prod(dim=dim+1, dtype=torch.half)
        return res
    ...
    def forward(self, feat, reshape_required=False, alpha=None):
        """
        Args:
        ...
        if self.cal_dim == "cin":
            ...
            weight = (w_alpha.permute(0,2,1,3,4).unsqueeze(5)*self.weight).reshape(-1, c_in//self.groups, kh, kw)
        ...
        # if weight.isnan().any() == True:
        #     print('in tada conv')
        #     raise Exception
        
        # if weight.isinf().any() == True:
        #     print('in tada conv')
        #     raise Exception
        bias = None
        if self.bias is not None:
        ...
        output = output.view(b,t,c_out,output.shape[-2],output.shape[-1]).permute(0,2,1,3,4)
        if hasattr(self, "bn_a") and hasattr(self, "bn_b"):
            # output = self.bn_a(output) + self.bn_b(self.cumprod(output))
