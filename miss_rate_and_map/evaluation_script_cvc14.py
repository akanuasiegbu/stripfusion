...
class KAISTPedEval(COCOeval):
    ...
    def __init__(self, kaistGt=None, kaistDt=None, iouType='segm', method='unknown'):
        ...
        self.params = CVC14Params(iouType=iouType)
        self.method = method
...
class CVC14Params(Params):
    ...
    def setDetParams(self):
        ...
        # self.HtRng = [[ 55, 1e5 ** 2], [50, 1e5 ** 2], [20, 1e5 ** 2]]
        # self.OccRng = [[0, 1], [0, 1], [2], [0, 1, 2]]
        # self.SetupLbl = ['Reasonable', 'Reasonable_small', 'Reasonable_occ=heavy', 'All']

        self.fppiThrs = np.array([0.0100, 0.0178, 0.0316, 0.0562, 0.1000, 0.1778, 0.3162, 0.5623, 1.0000])
        
        self.HtRng = [[55, 1e5 ** 2], [115, 1e5 ** 2], [45, 115], [1,45], [1, 1e5 ** 2]]
        self.OccRng = [[0, 1], [0], [0], [0], [0, 1, 2]]
        
        self.SetupLbl = ['Reasonable', 'Near', 'Medium', 'Far', 'All', ]

        self.bndRng = [5, 5, 635, 466]  # discard bbs outside this pixel range
...
def evaluate_night_subsets(test_annotation_file: str, user_submission_file: str or list, phase_codename: str = 'Multispectral'):
    """Evaluates the submission for a particular challenge phase and returns score
    ...
    for ind in range(0,5,1):
        ...
        eval_result['all'].params.imgIds = imgIds[690:]
...
if __name__ == "__main__":
    ...
    results = []
    for index, rstFile in enumerate(args.rstFiles):
        print('%'*40)
        print('%'*40)
        print(f'{index}:{rstFile}')
        if not args.night_albation:
            results.append(evaluate(args.annFile, rstFile, phase))
        else:
            results.append(evaluate_night_subsets(args.annFile, rstFile, phase))
            print(results[0][1])
            quit()
