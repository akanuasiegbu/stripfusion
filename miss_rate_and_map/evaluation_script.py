...
class KAISTPedEval(COCOeval):
    ...
    def __init__(self, kaistGt=None, kaistDt=None, iouType='segm', method='unknown', dataset_used = 'kaist'):
        '''
        ...
        if 'kaist' in dataset_used:
            params = KAISTParams(iouType=iouType)
        elif 'cvc14' in dataset_used:
            params = CVC14Params(iouType=iouType)
        else:
            raise ValueError("Invalid dataset_used provided") 
    ...
    def _prepare(self, id_setup):
        ...
        for gt in gts:
            ...
            gbox = gt['bbox']
            #what happens if already occluded since the ignore is techincally true in json??
            # tho true not initlized until below runs
            gt['ignore'] = 1 \
                ...
                for tind, t in enumerate(p.iouThrs):
                    for dind, d in enumerate(dt):
                        ...
                        for gind, g in enumerate(gt):
                            ...
                            if m > 0:
                                continue
                            # if dt matched to reg gt or ig gt , and on ignore gt,
...
class KAISTParams(Params):
    ...
    def setDetParams(self):
        ...
        self.HtRng = [[55, 1e5 ** 2], [115, 1e5 ** 2], [45, 115], [1,45], [1, 1e5 ** 2], [1, 1e5 ** 2], [1, 1e5 ** 2], [1, 1e5 ** 2]]
        self.OccRng = [[0, 1], [0], [0], [0], [0], [1], [2], [0, 1, 2]]
        self.SetupLbl = ['Reasonable', 'Near', 'Medium', 'Far', 'None', 'Partial', 'Heavy', 'All', ]
        
        # self.HtRng = [[20, 1e5 ** 2]]
        # self.OccRng = [[2]]
        # self.SetupLbl = ['heavy']
        
        # self.HtRng = [ [115, 1e5 ** 2]]
        # self.OccRng = [[0]]
        # self.SetupLbl = ['Near']
...
def evaluate(test_annotation_file: str, user_submission_file: str or list, phase_codename: str = 'Multispectral', dataset_used: str = 'kaist'):
    ...
    kaistEval = KAISTPedEval(kaistGt, kaistDt, 'bbox', method, dataset_used)
    ...
    if 'kaist' in dataset_used:
        day_images = imgIds[:1455]
        night_images = imgIds[1455:]
    elif 'cvc14' in dataset_used:
        day_images = imgIds[:690]
        night_images = imgIds[690:] # Note images is 1417 length
    else:
        raise ValueError("Invalid dataset_used provided") 
 

    eval_result['day'].params.imgIds = day_images
    eval_result['day'].evaluate(0)
    eval_result['day'].accumulate()
    MR_day = eval_result['day'].summarize(0)

    eval_result['night'].params.imgIds = night_images
...
def evaluate_night_subsets(test_annotation_file: str, user_submission_file: str or list, phase_codename: str = 'Multispectral', dataset_used: str = 'kaist'):
    """Evaluates the submission for a particular challenge phase and returns score
    This IS FOR Albation
    Parameters
    ----------
    test_annotations_file: str
        Path to test_annotation_file on the server
    ...
    for ind in range(0,8,1):
        ...
        print(f"Night Albation: {setName}")
        eval_result = {
            'all': copy.deepcopy(kaistEval),
            'day': copy.deepcopy(kaistEval),
            'night': copy.deepcopy(kaistEval),
        ...
        eval_result['all'].params.imgIds = night_images
...
def txt_to_missrate(txt_file, annFile, phase, dataset_used):
    find_mr = []
    cur = "cur_24_predictions_ct2_all.json"
    with open(txt_file, 'r') as file:
        for line in file:
            # import pdb; pdb.set_trace()
            # print(line)
            line = line.strip('\n')
            file_to_find = f'./runs/test/{line}/{cur}'
            print(file_to_find)
            if os.path.exists(file_to_find):
                find_mr.append(file_to_find)
            else:
                print('DNE')
            
    pdb.set_trace()
    results = [evaluate(annFile, rstFile, phase, dataset_used) for rstFile in find_mr]
    
    with open('output.txt', 'w') as file:
        for path_mr, result in zip(find_mr, results):
            
            mr_all = result['all'].summarize(0)
            mr_day = result['day'].summarize(0)
            mr_night = result['night'].summarize(0)
            recall_all = 1 - result['all'].eval['yy'][0][-1]
            
            file.write(f'{path_mr},{mr_all},{mr_day},{mr_night},{recall_all} \n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_txt', type=str, default='None')
    parser.add_argument('--night_albation', action='store_true', help='Night Albation for reviewers')
    args = parser.parse_args()

    phase = "Multispectral"
    if args.use_txt == 'None':
        ...
        end = str(path).split('s_')[-1]
        spath = str(path.parent)
        if args.multiple_outputs:
            # path = str(path.parent)
            rstFile_list = []
            for i in range(0,40):
                temp = f'cur_{i}_predictions_{end}'
                rstPath = spath + '/' + temp
        ...
        if not  args.night_albation:
            results = [evaluate(args.annFile, rstFile, phase, args.dataset_used) for rstFile in args.rstFiles]
        else:
            results = [evaluate_night_subsets(args.annFile, rstFile, phase, args.dataset_used) for rstFile in args.rstFiles]
            print(results[0][1])
            quit()
        ...
        for result in results:
            ...
            temp_list.append(   result['all'].summarize(0)      )
            temp_list.append(   result['day'].summarize(0)      )
            temp_list.append(   result['night'].summarize(0)    )
            
        if args.multiple_outputs:
            save_loc = str(path.parent) + '/' + f'missrate_{end}'
        else:
        ...
        txt_to_missrate(args.use_txt, args.annFile, phase, args.dataset_used)
