    ...
    for f in files:
        if os.path.isfile(f):
            with open(f, 'rb') as file:
                while chunk := file.read(8192):  # Read in chunks to handle large files efficiently
                    hasher.update(chunk)
    
    return hasher.hexdigest()  # Return a unique hash of all file contents
...
def create_dataloader_rgb_ir(path1, path2, imgsz, batch_size, stride, opt, temporal_stride, 
                                ...
                                temporal_mosaic=False, use_tadaconv=True, supervision_signal='lastframe', 
                                sanitized=True, mosaic=False):

    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    assert supervision_signal in ['lastframe', 'fullframes', 'midframe'], 'supervision_signal must be either lastframe, fullframes, or midframe'
    ...
    # # This allows me to load multiple batches as opposed to a single batch
    # # This is needed because of collate_fn which resizes dataset output into batch format    
    # if gframe == 0:
    #     lwindow = lframe*temporal_stride - temporal_stride + 1
    #     if lwindow != 1:
    #         num_of_images_in_one_sequence = len(np.arange(0,lwindow, temporal_stride))
    #     else:
    #         num_of_images_in_one_sequence = 1
    #     batch_size = batch_size*num_of_images_in_one_sequence
    # else:
    #     raise "Not implemented gframe > 0"

    # sampler = VIDBatchSampler(TrainTestSampler(dataset,is_training), batch_size, drop_last=False) #maybe should drop last on training
    # dataloader_kwargs = {
    #     "num_workers": nw,
    ...
    #     'collate_fn':LoadMultiModalImagesAndLabels.collate_fn4 if quad else LoadMultiModalImagesAndLabels.collate_fn_temporal
    # }
...
def img2label_paths(img_paths, dataset_used, sanitized=False):
    ...
    elif dataset_used == 'kaist_reliability_test':
        # refer to create_folder_testing_kaist.py
        # Kept bbox that would be used in reasonable eval
        if_lwir, if_visible = '/lwir/', '/visible/'
        sa, sb = os.sep + 'images' + os.sep, os.sep + 'labels_test_from_kaist_annot_reasonable' + os.sep  # /images/, /labels/ substrings
        res = ['txt'.join(x.replace(sa, sb, 1).replace(if_lwir, '/', 1).replace(if_visible, '/',1).rsplit(x.split('.')[-1], 1)) for x in img_paths]
        
    elif dataset_used == 'cvc14':
        sa, sb = os.sep + 'CVC-14' + os.sep, os.sep + 'CVC-14/labels_cvc14' + os.sep  # /CVC-14/, /CVC-14/labels
        res = ['txt'.join(x.replace(sa, sb, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]
    elif dataset_used == 'cvc14_alignv2':
        # this was the one using 67.5% of annotations absolute in training was v1, attempt at MBNet
        # v2 is the one using 91% of annotation in training similar to Causal 
        sa, sb = os.sep + 'CVC-14' + os.sep, os.sep + 'CVC-14/labels_cvc14_alignedv2' + os.sep  # /CVC-14/, /CVC-14/labels
        res = ['txt'.join(x.replace(sa, sb, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]
    elif dataset_used == 'cvc14_align_resizedv2':
        # this was the one using 67.5% of annotations absolute in training was v1, attempt at MBNET
        # v2 is the one using 91% of annotation in training similar to Causal 
        sa, sb = os.sep + 'CVC-14' + os.sep, os.sep + 'CVC-14/labels_cvc14_aligned_resizedv2' + os.sep  # /CVC-14/, /CVC-14/labels
        res = ['txt'.join(x.replace(sa, sb, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]
    elif dataset_used == 'cvc14_align_resizedv2_use_only_rgb_anns':
        if_fir, use_visible = 'FIR', 'Visible'
       
        sa, sb = os.sep + 'CVC-14' + os.sep, os.sep + 'CVC-14/labels_cvc14_aligned_resizedv2' + os.sep  # /CVC-14/, /CVC-14/labels
        res = ['txt'.join(x.replace(sa, sb, 1).replace(if_fir,use_visible, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]
    else:
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 ...
                 use_tadaconv=True, supervision_signal='lastframe', sanitized=True, mosaic=False):
        ...
        self.mosaic = True if self.temporal_mosaic else mosaic
        self.stride = stride
        ...
        if not self.is_training:
            ...
            if 'kaist' in dataset_used:
                ...
                raise 'Dataset not kaist, camel, cvc-14'
            ...
            for i, sequence in self.res_dict.items():
                if 'kaist' in dataset_used:
                    ...
                    raise 'Dataset not kaist, camel, or cvc-14'
                    
        # Check cache
        # Check rgb cache
        self.label_files_rgb = img2label_paths(self.img_files_rgb,dataset_used=self.dataset_used, sanitized=self.sanitized)  # labels
        ...
        # Check ir cache
        self.label_files_ir = img2label_paths(self.img_files_ir, dataset_used=self.dataset_used, sanitized=self.sanitized)  # labels
        # print(self.label_files)
        ...
        if cache_ir_path.is_file():
            ...
            if cache_ir['hash'] != get_hash(self.label_files_ir + self.img_files_ir) or 'version' not in cache_ir:  # changed
                print('Rebuilding Cache Thermal!!!!!!!!!!!!!')
        ...
        self.label_files_rgb = img2label_paths(cache_rgb.keys(), dataset_used=self.dataset_used, sanitized=self.sanitized)  # update
        ...
        self.label_files_ir = img2label_paths(cache_ir.keys(), dataset_used=self.dataset_used, sanitized=self.sanitized)  # update
    ...
    def photo_to_sequence(self, lframe, temporal_stride, gframe=0):
        ...
        for video in sorted_videos:
            video.sort() #in case someone forgets to sort video is always sorted
            ele_len = len(video)
            if self.is_training and (not self.sanitized) and ('kaist' in self.dataset_used):  
                ...
                if self.mode == 'gl':
                    for frame_num, frame in enumerate(video):
                        ...
                        if 'kaist'  in self.dataset_used:
                            nearby_strided_frames = self.get_frames_tublet(frame, temporal_stride, lframe, self.dataset_used, self.supervision_signal)
                        elif 'cvc14' in  self.dataset_used:
                            nearby_strided_frames = self.get_frames_tublet(frame, temporal_stride, lframe, self.dataset_used, self.supervision_signal, 
                                                                            video, frame_num, self.is_training)
                        else:
                            raise 'Not the kaist or CVC-14 dataset'
    ...
    @staticmethod
    def get_frames_tublet(frame, temporal_stride, lframe, dataset_used, supervision_signal='lastframe', video=None, frame_num=None, is_training=None):
        """
        ...
        Inputs:
            frame: string of the current frame being used 
            temporal_stride: Local Frames in a batch are strided by this amount
            lframe: Number of Local Frames in trajectory (Batch)
            dataset_used: 'kaist' or 'cvc14'
            supervision_signal: 'lastframe'
            video: the video is a list that contains the frame of interest, meaning that each element in the list is a str of image_path name
            frame_num: When sorted the frame number is the element in the order it appears in the video list
        Outputs:
            new_frames: frames in trajectory 
        """
        lwindow = lframe*temporal_stride - temporal_stride + 1
        
        with open("./miracle_update_cvc14_kaist/CVC14/labels_cvc14_aligned_resizedv2/cvc14_test_visible_resized.txt", "r") as file:
            lines = file.read().splitlines()
        frame_index_map = {path: idx for idx, path in enumerate(lines)}

        
        new_frames = []
        if 'kaist' in dataset_used:
            frame_num = int(frame[-9:-4])
            if supervision_signal == 'lastframe':
                # lframe*temporal_stride - temporal_stride + 1
                start_idx = frame_num+1-lwindow
                end_idx = frame_num+1
            else:
                raise 'not implemented'
            # elif self.supervision_signal == 'midframe':
            #     start_idx = frame_num - lwindow//2
            #     end_idx = frame_num + lwindow//2 + 1
            # print(frame_num, lwindow, start_idx, end_idx)
            # import pdb; pdb.set_trace()
            if start_idx < 0:
                # Repeats the earlier frame
                for i in range(lframe):
                    frame_path = os.path.join( frame[:-10] , 'I{:05d}'.format(frame_num) + '.jpg')
            ...
            if is_training:
                # Assumpution video is sorted so its in order and indexing gets perserves video order. (this was checked and validated)
                if frame_num + 1 < lwindow:
                    for i in range(lframe):
                        new_frames.append(frame)
                else:
                    frames_to_add = video[frame_num::-temporal_stride][:lframe][::-1]
                    new_frames = frames_to_add
                    assert len(new_frames) == lframe, 'frame length is less than lframe'
            else:
                frames_to_add = video[frame_num::-temporal_stride][:lframe][::-1]

                ############################### njotwani
                # Ensure frames do not cross video subset boundaries
                video_ranges = [
                    (0, 361), (362, 453), (454, 689), 
                    (700, 897), (898, 1054), (1055, 1206), (1207, 1245), (1245, 1417)
                ]
                
                frame_path = os.path.abspath(frame)
                frame_number = frame_index_map.get(frame_path, None)  # Extract frame number
                assert frame_number == frame_num
                # print("\n\n", frame_path, "\t", frame_number)
                valid_subset = None

                for subset in video_ranges:
                    if subset[0] <= frame_number <= subset[1]:
                        valid_subset = subset
                        break

                if valid_subset:
                    # Keep only frames that belong to the same subset
                    frames_to_add = [f for f in frames_to_add 
                            if frame_index_map.get(os.path.abspath(f), -1) in range(valid_subset[0], valid_subset[1] + 1)]

                new_frames = frames_to_add

                # Ensure new_frames length matches lframe
                if len(new_frames) < lframe:
                    if new_frames:  # If there are any frames, repeat the last valid frame
                        last_frame = new_frames[-1]
                    else:  # If `new_frames` is empty, default to the original frame being processed
                        last_frame = frame

                    while len(new_frames) < lframe:
                        new_frames.append(last_frame)

                if len(new_frames) != lframe:
                    print("\n\nlen new frames: ", len(new_frames))
                    print("lframe: ", lframe)
    ...
    def __getitem__(self, index):
        ...
        if self.supervision_signal in ['lastframe', 'midframe']:
            if self.supervision_signal == 'lastframe':
                index_to_look_at = -1
            elif self.supervision_signal == 'midframe':
                index_to_look_at = -2
            else:
                raise 'supervision_signal must be either lastframe or midframe'
        ...
        # Letterbox
            ...
            # img_rgb, ratio, pad = letterbox(imgs_rgb, shape, auto=False, scaleup=self.augment)
            # img_ir, ratio, pad = letterbox(imgs_ir, shape, auto=False, scaleup=self.augment)
            
            shapes = (h0, w0), ((h / h0, w / w0), pad)  # for COCO mAP rescaling

            # labels = self.labels_rgb[path].copy()
            if self.supervision_signal in ['lastframe', 'midframe']:
                labels = self.labels_rgb[self.res_dict[index][index_to_look_at]].copy()
                labels_ir = self.labels_ir[self.res_dict_ir[index][index_to_look_at]].copy()
                if labels.size:  # normalized xywh to pixel xyxy format
                    labels[:, 1:] = xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
                if labels_ir.size:
                    labels_ir[:,1:] = xywhn2xyxy(labels_ir[:,1:], ratio[0]*w, ratio[1]*h, padw=pad[0], padh=pad[1])
            elif self.supervision_signal == 'fullframes':
                raise 'Not implemented because labels_ir not defined'
                labels_list = []
                for index_to_look_at in range(len(self.res_dict[index])):
                    labels = [self.labels_rgb[i].copy() for i in self.res_dict[index]][index_to_look_at]
                    if labels.size:  # normalized xywh to pixel xyxy format
                        labels[:, 1:] = xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
                        # Here we use len(self.res_dict[index]) - index_to_look_at - 1 instead of index_to_look_at
                        # to reverse the order of the labels Because the images are loaded in reverse order
                    labels_list.append(np.insert(labels, 0, len(self.res_dict[index]) - index_to_look_at - 1, axis=1))
                    
                labels = np.concatenate(labels_list, 0)
            else:
                raise 'supervision_signal must be either lastframe or fullframes'
        ...
        if self.augment:
            ...
            # augment_hsv(imgs_rgb, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
            # augment_hsv(imgs_ir, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
        ...
        if self.supervision_signal in ['lastframe', 'midframe']:
            ...
            if nL_ir:
                labels_out_ir[:, 1:] = torch.from_numpy(labels_ir)

        elif self.supervision_signal == 'fullframes':
            raise 'Did not fix'
            nL = len(labels_rgb)  # number of labels
            if nL:
                labels_rgb[:, 2:6] = xyxy2xywh(labels_rgb[:, 2:6])  # convert xyxy to xywh
                labels_rgb[:, [3, 5]] /= imgs_rgb.shape[0]  # normalized height 0-1
                labels_rgb[:, [2, 4]] /= imgs_rgb.shape[1]  # normalized width 0-1
        ...
        # Reshaping to channels x frames x height x width (irrespective of tadaconv) NJOTWANI
        ...
        return torch.from_numpy(imgs_all), labels_out, labels_out_ir, path, shapes
        # return torch.from_numpy(imgs_rgb), torch.from_numpy(imgs_ir), labels_out, path, shapes, self.num_of_images_in_one_sequence
    ...
    def collate_fn(batch):
        img, label, label_ir, path, shapes = zip(*batch)  # transposed
        ...
        return torch.stack(img, 0), torch.cat(label, 0), torch.cat(label_ir, 0), path, shapes
    ...
    def collate_fn4(batch):
        ...
        img4, label4, path4, shapes4 = [], [], path, shapes[:n]
...
def load_image_rgb_ir_temporal(self, index):
    ...
    for path_rgb, path_ir in zip(paths_rgb, paths_ir):
        ...
        assert imgs_rgb is not None, 'Image RGB Not Found ' + path_rgb
        assert imgs_ir is not None, 'Image IR Not Found ' + path_ir
...
def load_mosaic_RGB_IR_temporal(self, index, delta, temporal_mosaic=True):
    ...
    for i, index in enumerate(indices):
        ...
        if self.supervision_signal in ['lastframe', 'midframe']:
            ...
            if temporal_mosaic or i == 0: #when i=0 temporal sequence
                ...
                labels_rgb = self.labels_rgb[self.res_dict[index][index_to_look_at]].copy() # i here is path

                segments_ir = self.segments_ir[index].copy()
                labels_ir = self.labels_ir[self.res_dict_ir[index][index_to_look_at]].copy() # i here is path

            else:
                segments_rgb = self.segments_rgb[index[0]].copy() #empty
                labels_rgb = [self.labels_rgb[self.img_files_rgb[i]].copy() for i in index][index_to_look_at] # i here in int

                segments_ir = self.segments_ir[index[0]].copy()
                labels_ir = [self.labels_ir[self.img_files_ir[i]].copy() for i in index][index_to_look_at] # i here is int
            ...
            if temporal_mosaic or i == 0: #when i=0 temporal sequence
                ...
                segments_rgb = self.segments_rgb[index[0]].copy() #empty
                segments_ir = self.segments_ir[index[0]].copy()
            if len(segments_rgb) > 0:
                segments_rgb = [xyn2xy(x, w, h, padw, padh) for x in segments_rgb]
            if len(segments_ir) > 0:
                segments_ir = [xyn2xy(x, w, h, padw, padh) for x in segments_ir]
            ...
            if temporal_mosaic or i == 0: #when i=0 temporal sequence
                for index_to_look_at in range(len(self.res_dict[index])):
                    ...
                    # Here we use len(self.res_dict[index]) - index_to_look_at - 1 instead of index_to_look_at
                    # to reverse the order of the labels Because the images are loaded in reverse order
                    labels4_rgb.append(np.insert(labels_rgb, 0, len(self.res_dict[index]) - index_to_look_at - 1, axis=1))
                    labels4_ir.append(np.insert(labels_ir, 0, len(self.res_dict[index]) - index_to_look_at - 1, axis=1))
            else:
                for index_to_look_at in range(len(index)):
                    labels_rgb = [self.labels_rgb[self.img_files_rgb[i]].copy() for i in index][index_to_look_at] # i here in int
                    labels_ir = [self.labels_ir[self.img_files_ir[i]].copy() for i in index][index_to_look_at] # i here is int
                    if labels_rgb.size:
                        labels_rgb[:, 1:] = xywhn2xyxy(labels_rgb[:, 1:], w, h, padw, padh)  # normalized xywh to pixel xyxy format
                    ...
                    # Here we use len(self.res_dict[index]) - index_to_look_at - 1 instead of index_to_look_at
                    # to reverse the order of the labels Because the images are loaded in reverse order
                    labels4_rgb.append(np.insert(labels_rgb, 0, len(index) - index_to_look_at - 1, axis=1))
                    labels4_ir.append(np.insert(labels_ir, 0, len(index) - index_to_look_at - 1, axis=1))       
        else:
            raise 'Unsupport supervision signal'
                
            

    # # Concat/clip labels

    start = 2 if self.supervision_signal == 'fullframes' else 1
    ...
    img4_rgb_new, img4_ir_new, labels4_rgb_new, labels4_ir_new = random_perspective_rgb_ir_temporal(img4_rgb, img4_ir, labels4_rgb, labels4_ir,
                                                    ...
                                                    delta = delta,
                                                    supervision_signal = self.supervision_signal )  # border to remove

    # labels4_ir_new = labels4_rgb_new
...
def random_perspective_rgb_ir_temporal(imgs_rgb, imgs_ir, targets_rgb=(),targets_ir=(), segments_rgb=(), segments_ir=(),
                              degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0), delta=0, supervision_signal='lastframe'):
    ...
    scale_uniform = random.uniform(1 - scale, 1 + scale)
    # s = 2 ** random.uniform(-scale, scale)
    R[:2] = cv2.getRotationMatrix2D(angle=a, center=(0, 0), scale=scale_uniform)
    ...
    if supervision_signal == 'lastframe':
        ...
        raise 'Need to select right M, and apply to corresponding label coordinates'
    # assmpution is that width and height of RGB and thermal are the same
    targets_out = transform_label_coordinates(targets_rgb, M, scale_uniform, segments_rgb, perspective, width, height)
    targets_out_ir = transform_label_coordinates(targets_ir, M, scale_uniform, segments_ir, perspective, width, height)
    ...
    return imgs_rgb, imgs_ir, targets_out, targets_out_ir

def transform_label_coordinates(targets, M, scale_uniform, segments, perspective, width, height):
    """
    targets: either rgb labels or thermal labels
    M: transformation matrix (using M = O @ M_orginal )
    scale_uniform: scale label corresponding to scaled image
    segments: either rgb segments or thermal segements
    perspective: perspective tranformation value
    width: rgb width or thermal height
    height: rgb height or thermal height
    """
    n = len(targets)
    ...
    if n:
        ...
        start_i = 2 if targets_shape == 6 else 1
        # filter candidates
        i = box_candidates(box1=targets[:, start_i:end_i].T * scale_uniform, box2=new.T, area_thr=0.01 if use_segments else 0.10)
        targets = targets[i]
        targets[:, start_i:end_i] = new[i]
    return targets
...
def extract_boxes(path='../coco128/'):  # from utils.datasets import *; extract_boxes('../coco128')
    ...
    # TODO: Warning: The datasets_vid/extract_boxes function is hard coded with sanitized = True, which may not be what the user wants
    print("Warning: The datasets_vid/extract_boxes function is hard coded with sanitized = True, which may not be what the user wants")
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in img_formats:
            ...
            # labels
            lb_file = Path(img2label_paths([str(im_file)])[0], sanitized=True)
...
def autosplit(path='../coco128', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    """
    # TODO: Warning: The autosplit function is hard coded with sanitized = True, which may not be what the user wants
    print("Warning: The autosplit function is hard coded with sanitized = True, which may not be what the user wants")
    path = Path(path)  # images dir
    ...
    for i, img in tqdm(zip(indices, files), total=n):
        if not annotated_only or Path(img2label_paths([str(img)], sanitized=True)[0]).exists():  # check label
