##############################################################################################################
#Part 1, this creates the txt file with the list of images to use
import re
...
if args.even_val:
    ...
    root_dir = f"{prefix}/kaist{'small' if args.small else ''}_video_sanitized{'_whole' if args.whole else ''}_even_val" + ("_midframe" if args.midframe else "" + "_nc1" if args.nc == 1 else "")
else:
    ...
    root_dir = f"{prefix}/kaist{'small' if args.small else ''}_video_sanitized{'_whole' if args.whole else ''}" + ("_midframe" if args.midframe else "" + "_nc1" if args.nc == 1 else "")
...
for filename in tqdm(sorted(os.listdir(data_path))):
    ...
    # if lwindow > int(filename[1:])+1: # dont need this anymore as dataloader code takes care of it
    #     print(f'skipping {setid}/{vid}/{filename}')
    #     continue
    # ignore = True
    # with open(f"{data_path}/{setid}_{vid}_{filename}.txt", "r") as file:
    #     firstline = file.readline()
    #     assert "% bbGt version=3" in firstline, f"firstline contains data {data_path}/{setid}_{vid}_{filename}.txt"
    #     line = file.readline()
    #     while line:
    #         line = line.split()
    #         if line[0] == "person" :
    #             ignore = False
    #         # else:
    #         #     if f"{data_path}/{setid}_{vid}_{filename}.txt" not in ignored_file:
    #         #         ignored_file.add(f"{data_path}/{setid}_{vid}_{filename}.txt")
    #             # print(f"{data_path}/{setid}_{vid}_{filename}.txt")
    #         line = file.readline()
...
def write_to_file(file, dataset, modal):
    ...
    for set_vid, file_name in dataset:
        ...
        # if start_idx < 0: 
        #     print(f'skipping {set_vid}/{modal}/I{end_idx:05d}')
        #     continue
        end_idx = frame_num
...
# print(f" the number of ignored images is {len(ignored_file)}")

##############################################################################################################
# Part 2 creates another folder that contains only labels that will be used for training, Need to come back and clean up code

# source = "/mnt/workspace/datasets/kaist-cvpr15/sanitized_annotations/sanitized_annotations"

# target = "/mnt/workspace/datasets/kaist-cvpr15/sanitized_annotations/sanitized_annotations_format_all"
...
# '''
# % bbGt version=3
# person 170 212 19 46 0 0 0 0 0 0 0
# person 188 213 19 47 0 0 0 0 0 0 0
# person 327 213 11 26 0 0 0 0 0 0 0
# person 365 207 11 26 1 0 0 0 0 0 0
# '''
# for filename in tqdm(os.listdir(source)):
#     with open(f"{source}/{filename}", "r") as file:
#         firstline = file.readline()
#         line = file.readline()
#         while line:
#             line = line.split()
...
#                 with open(f"{target}/{filename}", "a") as targetfile:
